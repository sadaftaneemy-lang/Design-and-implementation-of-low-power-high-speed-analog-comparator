...
**It includes two modes of operation :** Reset Phase and Comparison phase. Block diagram shows the operation of the comparator. Figure 3 shows the operation of a conventional single-tail comparator. In the reset phase (CLK = 0), the tail transistor is OFF and output nodes are precharged.M7 and M8 pull the internal nodes to a fixed 
